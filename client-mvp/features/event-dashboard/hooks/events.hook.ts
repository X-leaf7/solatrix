...
import { useEventsContext } from '../provider'
import { EventTiming } from '@/data/types/event'

export function useEvents(eventTiming: EventTiming) {
  const { liveEvents, upcomingEvents, previousEvents, allEvents } = useEventsContext()
